...
int main(int argc, char** argv) {
    ...
    int vector_size_bytes = sizeof(int) * DATA_SIZE;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_vadd = cl::Kernel(program, "kernel_rtl", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_vadd.setArg(3, DATA_SIZE));
    ...
    int match = 0;
    for (int i = 0; i < DATA_SIZE; i++) {
        std::cout << "FPGA Result = " << source_hw_results[i] << std::endl;
